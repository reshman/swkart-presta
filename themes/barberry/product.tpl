...
{if $errors|@count == 0}
    ...
    <header class="entry-header">
        <h1 class="entry-title title-page">{l s='The Product'}</h1>
        <h2 class="sub-title-page">{l s='This is your product description!'}</h2>
    ...
    <div class="container fullwidth pside_fullwidth"> 
        ...
        <div class="product_navigation mobiles"> 
                            <div class="clearfix"></div>
                            <h1 itemprop="name" class="product_title entry-title">{$product->name|escape:'html':'UTF-8'}</h1>
                        ...
                        <div itemprop="offers" itemscope itemtype="http://schema.org/Offer"  class="summary">
                            {if $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
                                <p id="old_price"class="price{if (!$product->specificPrice || !$product->specificPrice.reduction) && $group_reduction == 0}  hidden {/if}">
                                    {if $priceDisplay >= 0 && $priceDisplay <= 2}
                                        {hook h="displayProductPriceBlock" product=$product type="old_price"}
                                        <span id="old_price_display " class="amount">{if $productPriceWithoutReduction > $productPrice}{convertPrice price=$productPriceWithoutReduction}{/if}</span>
                                ...
                                <p class="our_price_display price" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                                    ...
                                    {if $priceDisplay >= 0 && $priceDisplay <= 2}
                                        <span id="our_price_display" class="amount" itemprop="price">{convertPrice price=$productPrice}</span>
        ...
        <div class="poduct_details_left_col span6">
            {if $product->new} 
                <span class="pro_new">{l s='New!'}</span>
            {/if}
            {if $product->on_sale}
      ...
      <script type="text/javascript">
                    ...
                    <div class='slider'>
                        {if $have_image}
                            ...
                            {foreach from=$images item=image name=thumbnails}
                                ...
                                <div class="item  main-image_{$product->id}">
                                    ...
                                    <a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}" 
                                       class="fresco" data-fresco-group="product-gallery" data-fresco-options="fit: 'width'">
                                        <span itemprop="image">
                                            <img width="570" height="750" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}" class="attachment-shop_single wp-post-image"title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}"  />
                                        </span>
                                        <span class="tdl_zoom">
                ...
                <link rel="image_src" href="{$link->getImageLink($product->link_rewrite, $imageIds, 'cart_default')|escape:'html':'UTF-8'}" />
                        ...
                        {if isset($images)}
                            {foreach from=$images item=image name=thumbnails}
                                ...
                                <div class="button">
                                        ...
                                        <img width="114" height="150" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'cart_default')|escape:'html':'UTF-8'}" class="attachment-shop_thumbnail wp-post-image" alt="" />
        ...
        <div itemprop="description" class="product_description">
            {if $product->description_short || $packItems|@count > 0}
                        ...
                        <form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
                            ...
                            <div class="single_variation_wrap" style="">
                                ...
                                {if $PS_STOCK_MANAGEMENT}
                                    <!-- availability -->
                                    <div id="availability_statut"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) || ($product->quantity > 0 && !$product->available_now) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if} class="add_to_cart_simple">
                                        {*<span id="availability_label">{l s='Availability:'}</span>*}
                                        <p  class="{if $product->quantity <= 0 && !$allow_oosp}warning_inline stock out-of-stock{else}stock in-stock{/if}">{if $product->quantity <= 0}{if $allow_oosp}{$product->available_later}{else}{l s='Out of stock'}{/if}{else}{$product->quantity|intval} {$product->available_now}{/if}</p>
                                    </div>
                ...
                <div class="product_meta">
                    ...
                    <span id="product_reference"{if empty($product->reference) || !$product->reference} style="display: none;"{/if} class="sku_wrapper">
                        <strong>{l s='SKU:'}</strong>
                        <span class="editable sku" itemprop="sku">{if !isset($groups)}{$product->reference|escape:'html':'UTF-8'}{/if}</span>
                    </span>
                    {if $product->condition}
                        <p id="product_condition">
                            <strong>{l s='Condition:'}</strong>
