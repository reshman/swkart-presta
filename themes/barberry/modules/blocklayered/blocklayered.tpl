...
{if $nbr_filterBlocks != 0}
    <div id="layered_block_left" class="widget-container widget_price_filter">
        
        <div class="block_content">
            <form action="#" id="layered_form" class="widget">
                <div>
                    ...
                    {foreach from=$filters item=filter}
                        {if isset($filter.values)}
                            {if isset($filter.slider)}
                                ...
                                {else}
                                    <div class="layered_filter widget widget woocommerce widget_layered_nav">
                                    {/if}
                                    <h1 class="layered_subtitle_heading widget-title">
                                                    ...
                                                    {if $value.nbr || !$hide_0_values}
                                                        <li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} col-lg-12">
                                                            ...
                                                            <label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
                                                                {if !$value.nbr}
                                                                    {$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<small class="count"> {$value.nbr}</small>{/if}
                                                                {else}
                                                                    <a href="{$value.link}"{if $value.rel|trim != ''} rel="{$value.rel}"{/if}>{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<small class="count"> {$value.nbr}</small>{/if}</a>
                                                                {/if}
                                                            </label>
                                                        </li>
                                                    {/if}
